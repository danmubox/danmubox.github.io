//用法：
//1.因为js并不能获取系统中所有字体，因为没有权限.只有IE可以
//2.因此创建一个字体列表根据像素差逐一匹配也算是一个解决办法了
...
function pushIfSupport(data) {
    if (isSupportFontFamily(data.en)) {
...
function getFontFamilyList() {
    ...
    if (fontFamilyList.length == 0) {
        for (var i in dataFont) {
            dataFont[i].forEach(pushIfSupport);
        }
    }
